...
typedef enum {
    ...
    R9, R10, R11, R12, R13, R14, R15, INVALID_REG
} register_num;
...
/* get_register: gets a register name as a string and return its number, or the
   speciel value for invalid register if not one of r0 - r15 */
int get_register(char source_word[MAXLINE])
{
    ...
    for (i = 0; (i < invalid_reg) && 
        (strcmp(source_word, registers[i].name) != 0); i++)
        ;
...
/* get_brackets: return the register number stored inside the brackets [], or
   the a speciel value for invalid register if not one of r0 - r15 */
    				   ...
    				   char *second_op, char *cmd, src_op_line *srcline)
{
    ...
    codeword[address]->funct_nd_ops 
    	|= opcodes[get_opcode(cmd)].op_funct << FUNCT_FIELD; /*set funct field*/
    ...
    if (strlen(second_op) != 0) { /* two operands so first operand is source*/
        ...
        if (mode == REG_DIRECT) {
            ...
            codeword[address]->funct_nd_ops
            	|= register_number << SRC_REGISTER_FIELD;
        }
        ...
        if (mode == REG_DIRECT) {
            ...
            if ((register_number = registers[get_register(second_op)].reg_num)
            ...
            codeword[address]->funct_nd_ops
            	|= register_number << TARGT_REGISTER_FIELD; 
...
int get_opcode(char source_word[MAXLINE])
{
    ...
    for (i= 0; i < (sizeof(opcodes)/sizeof(opcodes[0])) && 
        (strcmp(source_word, opcodes[i].name) != 0) ; i++)
        ;
